...
import purchasems.api.model.ErrorResponse;
...
public class CustomControllerAdvice {
  ...
  public ResponseEntity<ErrorResponse> handleValidationExceptions(Exception e, WebRequest request) {
    ...
    log.info("E_PUR_400 | {}", e.getMessage());

    return new ResponseEntity<>(
        new ErrorResponse("E_PUR_400", "Missing required request parameters"),
  ...
  public ResponseEntity<ErrorResponse> handleExceptions(Exception e, WebRequest request) {
    ...
    log.info("E_PUR_500 | {} : {}", e.getClass(), e.getMessage());

    return new ResponseEntity<>(
        new ErrorResponse("E_PUR_500", "An Internal Server Error occurred"),
        HeadersUtil.defaultHeaders(request.getHeader("X-Correlation-Id")),
        status);
  }
  ...
  public ResponseEntity<ErrorResponse> handleNoHandlerFound(Exception e, WebRequest request) {
    ...
    log.info("E_PUR_404 | {}", e.getMessage());

    return new ResponseEntity<>(
        new ErrorResponse("E_PUR_404", "Resource Not Found"),
        HeadersUtil.defaultHeaders(request.getHeader("X-Correlation-Id")),
        status);
  }
}
